# RECORTE A — Dossiê Completo (v3 — revisado contra as fontes + referências fundamentais incorporadas)
...
Registro consolidado do Recorte A (projeto de mestrado, PPGCC UFMA). Esta é a **versão 3**: a v2 revisou frase a frase os 41 artigos originais; a v3 incorpora **9 referências fundamentais adicionais** (pasta `Ref Adicionadas`), todas lidas integralmente, e o achado técnico QA-LoRA×inferência. Verificação completa em `ANALISE_CRITICA_RECORTE_A.md` e `LEITURA_INTEGRAL_LOG.md`.
...
## Adições da v2 para a v3

12. **9 referências fundamentais incorporadas e lidas na íntegra** (ReVeal/Chakraborty 2020, VulDeePecker 2018, LineVul 2022, Steenhoek 2024, Risse & Böhme 2024, LLM.int8()/Dettmers 2022, Gholami 2021 survey, Zeus/You 2022, Mao 2024 survey de LoRA) — ver §12.6.
13. **Achado QA-LoRA×inferência** (§7.4): QLoRA puro **não** dá ganho de inferência (desquantiza na hora de inferir); usar QA-LoRA ou mesclar+re-quantizar (AWQ/GPTQ). Refletido no Estágio 3 (§9).
14. **Reforço da lacuna 5 (robustez/OOD):** três novas fontes convergentes — Risse & Böhme (modelos não distinguem vuln/patch; overfit a features irrelevantes), Steenhoek (6/27 bugs vs 27/27 humanos), DiverseVul (F1 49%→9,4% em projetos não vistos).
15. **Nota de suficiência bibliográfica:** com estas adições, o corpus (50 artigos) cobre os fundamentos de detecção, contaminação, quantização, PEFT e energia — considerado **suficiente para a qualificação**; ampliar só conforme a orientação exigir.
...
- **⚠️ QLoRA ≠ ganho de inferência (ponto crítico de desenho):** o QLoRA quantiza a base em 4 bits só para *treinar* o adapter; na inferência ele **desquantiza** o modelo de volta à alta precisão, então **não há economia de inferência** por si só (Mao et al., survey de LoRA, 2024). Para ganho real na inferência é preciso **QA-LoRA** (adapter na mesma precisão do modelo quantizado, sem desquantizar) ou **mesclar o adapter e re-quantizar (AWQ/GPTQ) o modelo final**. Isso conecta diretamente os Estágios 2 e 3 do projeto.
...
### 12.6. Referências fundamentais adicionais (INCORPORADAS ao corpus — lidas integralmente)

Adicionadas em set/2026 (pasta `Ref Adicionadas`) e lidas na íntegra; preenchem os fundamentos que faltavam:
- **Chakraborty, S. et al. ReVeal — "Deep Learning Based Vulnerability Detection: Are We There Yet?"** IEEE TSE, 2021. [rev. por pares] — queda >50% no mundo real; duplicação até 68%; features irrelevantes. Fundamento de contaminação anterior ao PrimeVul.
- **Li, Z. et al. VulDeePecker.** NDSS, 2018. [rev. por pares] — 1º sistema DL (code gadgets + BLSTM). Fundamento histórico.
- **Fu, M.; Tantithamthavorn, C. LineVul.** MSR, 2022. [rev. por pares] — CodeBERT line-level; F1 0,91 no Big-Vul (número que o PrimeVul depois derruba).
- **Steenhoek, B. et al. A Comprehensive Study of the Capabilities of LLMs for Vulnerability Detection.** 2024. [preprint] — 11 LLMs; acurácia balanceada 0,5–0,63; 6/27 bugs vs 27/27 humanos.
- **Risse, N.; Böhme, M. Uncovering the Limits of Machine Learning for Automatic Vulnerability Detection.** USENIX Security, 2024. [rev. por pares] — overfitting a features irrelevantes; falha OOD; modelos não distinguem vuln/patch. Central para a lacuna 5.
- **Dettmers, T. et al. LLM.int8().** NeurIPS, 2022. [rev. por pares] — 8-bit sem perda até 175B; outliers emergentes (~6,7B); base do bitsandbytes/NF4.
- **Gholami, A. et al. A Survey of Quantization Methods for Efficient Neural Network Inference.** 2021. [survey] — taxonomia (QAT/PTQ, uniforme/não-uniforme, granularidade, precisão mista).
- **You, J.; Chung; Chowdhury. Zeus: Understanding and Optimizing GPU Energy Consumption of DNN Training.** NSDI, 2022. [rev. por pares] — metodologia ETA/NVML (é de TREINO; usar o método, não os números).
- **Mao, Y. et al. A Survey on LoRA of Large Language Models.** Frontiers of Computer Science, 2024. [survey] — taxonomia de LoRA; **QLoRA vs QA-LoRA** para inferência (ver §7.4/§9).
- *(Pendente, opcional: uma referência formal para GGUF/llama.cpp — é um formato/ferramenta, não paper.)*
